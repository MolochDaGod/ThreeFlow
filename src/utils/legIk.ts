/**
 * Fleet SSOT copy of Open `artifacts/animator/src/three/anim/legIk.ts`.
 * One FootGrounder per kit. Apply AFTER mixer.update. Same terrain as snapObjectToTerrain.
 */
...
export function findLegChain(
  ...
  if (!lower) {
    root.traverse((n) => {
      ...
      const nm = n.name;
      if (!sideRe.test(nm) || toeRe.test(nm)) return;
      if (n !== upper && n !== foot && legRe.test(nm) && !upperRe.test(nm))
        lower = n;
    });
  }
